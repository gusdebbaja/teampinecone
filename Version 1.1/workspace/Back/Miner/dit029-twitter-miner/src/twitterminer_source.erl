...
twitter_example() ->
  ...
  T = spawn_link(fun () ->
        ...
        %Restart the miner if an error occurs.
     % after 1000 ->
           % twitter_example(),       %Restart the miner if an error occurs.
  ...
  case T of
    {parsed_tweet, L, B, _} ->
      ...
      case extract(<<"text">>, L) of
        {found, TT} -> parser:save_tweet(unicode:characters_to_list(TT,unicode));
